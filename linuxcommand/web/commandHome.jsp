...
$(function() {
	...
	$(".addForm").submit(function() {
		if(checkEmpty("inputCommand", "命令")) return true;
		if(checkEmpty("inputFunction", "作用")) return true;
		return false;
	});
	/*
	$(".commandTR").mouseenter(function() {
		var id = $(this).attr("id");
		$("img#"+id).show(1000);
	});
	$(".commandTR").mouseleave(function() {
		var id = $(this).attr("id");
		$("img#"+id).toggle(1000);
	});
	*/
	$(".commandTR").click(function() {
		var url = location.href;
		var cid = $(this).attr("cid");
		location.href = url + "Detail?cid=" + cid;
	});
});
...
<body>	
	...
	<div class="manageDiv">
		...
		<form class="addForm" method="post" action="command_add">
			<div class="hidden addCommandDiv">
				<img class="returnImg" src="images/return.png">
				<div class="addCommandOfCommand">
					<span>命令</span> <input type="text" class="inputCommand" name="inputCommand" 
						id="inputCommand" autocomplete="off">
				</div>
			...
			<table class="table table-striped table-hover commandTable">
				...
				<tbody>
					<c:forEach items="${cs}" var="c">
						<tr class="commandTR" cid="${c.id}">
							<td>${c.name}</td>
							<td>${c.func}</td>
							<!--
							<td style="vertical-align: middle;"><a href="command_listDetail?cid=${c.id}">
								<img class="detailImg" id="${c.id}" width="16px" height="16px" src="images/detail.png">
							</a></td>
							-->
							<td style="vertical-align: middle;"><a class="hidden deleteCommand" href="command_delete?id=${c.id}">
